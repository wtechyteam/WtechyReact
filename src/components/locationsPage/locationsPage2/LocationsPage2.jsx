...
const LocationsPage2 = ({ pincodes }) => {
  ...
  return (
    <section className="sectionPadding locationPage">
      <div className="container">
        <div
          className="sectionTopInfo text-center"
          data-aos="fade-up"
        ...
        <div className="container">
          <div className={`row `}>
            <div className="col-md-5">
              <section className="mapSection" margin="25px">
                <iframe
                  src="https://www.google.com/maps/d/u/0/embed?mid=10PGqyXec9v0O-7Kn4b0RU4kEgtNLI8E&ehbc=2E312F&noprof=1"
                  width="520"
                  height="480"
                ></iframe>
              </section>
            ...
            <div className="col-md-7">
              ...
              <table className="table" margin="20px">
              <tbody>
  {[...Array(Math.ceil(pincodes.length / 4))].map((item, rowIndex) => (
    <tr key={rowIndex}>
      {pincodes
        .slice(rowIndex * 4, rowIndex * 4 + 4)
        .map((pincode, index) => {
          // Normalize pincode to replace consecutive spaces with a single dash
          const normalizedPincode = pincode.trim().replace(/\s+/g, "-");
          return (
            <td key={index}>
              <a href={`/seo-services-${normalizedPincode}`}>{pincode}</a>
            </td>
          );
        })}
    </tr>
